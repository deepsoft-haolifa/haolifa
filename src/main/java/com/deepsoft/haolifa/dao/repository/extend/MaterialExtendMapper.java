...
public interface MaterialExtendMapper {
    ...
    @Update("update material set current_quantity=current_quantity+#{quantity},version=version+1 where graph_no=#{graphNo} and version=#{version}")
    int updateCurrentQuantity(@Param("graphNo") String graphNo, @Param("quantity") int quantity, @Param("version") int version);

    @Update("update material set lock_quantity=lock_quantity+#{quantity},version=version+1 where graph_no=#{graphNo} and version=#{version}")
    int updateLockQuantity(@Param("graphNo") String graphNo, @Param("quantity") int quantity, @Param("version") int version);
